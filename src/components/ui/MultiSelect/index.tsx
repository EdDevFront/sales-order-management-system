...
export default function MultiSelect({
  ...
  return (
    <div ref={containerRef} className="relative w-full">
      ...
      >
        <span className={selectedCount === 0 ? "text-zinc-400" : "text-zinc-900 dark:text-zinc-100"}>
          {triggerLabel}
        </span>
      ...
      {isOpen && (
        <div className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md border border-zinc-200 bg-white p-1.5 shadow-lg dark:border-zinc-800 dark:bg-zinc-900">
          {options.length === 0 ? (
            <p className="px-2 py-3 text-xs text-zinc-400 italic text-center">Nenhuma opção disponível</p>
          ) : (
            options.map((opt) => {
              ...
              return (
                <label
                  key={opt.value}
                  className={`flex items-center gap-2.5 rounded-md px-3 py-2.5 sm:py-2 cursor-pointer transition-all text-sm ${
                    isSelected
