...
const mockOrders = [
  ...
  },
  {
    ...
    items: [{ itemId: "item-3", quantity: 5 }],
    status: "EM_TRANSPORTE" as const,
    createdAt: "2026-07-03T10:00:00.000Z",
  },
  {
    id: "so-4",
    customerId: "cust-3",
    transportTypeId: "trans-3",
...
describe("Dashboard", () => {
  ...
  it("CT-DASH-07: shows filtered empty state", async () => {
    renderComponent(
      createTestStore({ status: "ENTREGUE", clientId: "cust-2" }),
    );
    await waitFor(
      () =>
        expect(
          screen.getByText("Nenhum resultado corresponde aos filtros ativos."),
        ).toBeInTheDocument(),
      { timeout: 5000 },
    );
  });
