...
export function orderStatusVariant(status: SalesOrderStatus): string {
  return (
    STATUS_VARIANT_MAP[status] ?? "bg-zinc-100 text-zinc-700 border-zinc-200"
  );
}
...
export function OrderStatusBadge({ status }: OrderStatusBadgeProps) {
  return (
    <span
      className={`rounded-full px-2.5 py-1 text-xs font-semibold border ${orderStatusVariant(status)}`}
    >
