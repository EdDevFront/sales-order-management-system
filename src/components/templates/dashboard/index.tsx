...
export default function Dashboard() {
  ...
  const { data: transports = [], isLoading: loadingTransports } = useQuery({ queryKey: ["transports"], queryFn: fetchTransportTypes });
    ...
    <div className="space-y-8">
          ...
          return (
            <div key={idx} className={`rounded-xl border p-5 flex items-center justify-between ${card.bg}`}>
              <div>
                ...
                {isLoading ? <Skeleton className="h-8 w-16 mt-1" /> : <p className="text-2xl font-bold text-zinc-950 dark:text-white mt-1">{card.val}</p>}
              </div>
      ...
      <DataTable>
        ...
        <DataTable.Body
          ...
          isFilteredEmpty={!isLoading && orders.length > 0 && filteredOrders.length === 0}
          onClearFilters={handleClearFilters}
          colSpan={DASHBOARD_COLUMNS.length}
          skeletonRows={<DataTable.SkeletonRows widths={DASHBOARD_SKELETON_WIDTHS} />}
        >
