...
const mockRepos = jest.requireMock<
  typeof import("@/infrastructure/repositories/mockRepositories")
>("@/infrastructure/repositories/mockRepositories");
...
const logs: AuditLog[] = [
  {
    ...
    entityId: "so-1",
    nextState: JSON.stringify({ id: "so-1" }),
  },
  {
    id: "a2",
    ...
    entityAffected: "SALES_ORDER",
    entityId: "so-1",
    previousState: JSON.stringify({ status: "CRIADA" }),
    nextState: JSON.stringify({ status: "PLANEJADA" }),
  },
  {
    id: "a3",
    ...
    entityAffected: "SALES_ORDER",
    entityId: "so-1",
    previousState: JSON.stringify({ status: "PLANEJADA" }),
  ...
  },
  {
    ...
    entityAffected: "SALES_ORDER",
    entityId: "so-1",
    previousState: JSON.stringify({ transportTypeId: "trans-1" }),
    nextState: JSON.stringify({ transportTypeId: "trans-2" }),
  },
  {
    id: "a5",
    ...
    entityAffected: "SALES_ORDER",
    entityId: "so-2",
    nextState: JSON.stringify({ id: "so-2" }),
  },
];
...
describe("Audit", () => {
  ...
  it("CT-AUD-01: renders CREATE_ORDER badge", async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.getAllByText("CREATE_ORDER").length,
  ...
  it("CT-AUD-02: renders UPDATE_STATUS badge", async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.getAllByText("UPDATE_STATUS").length,
        ).toBeGreaterThanOrEqual(1),
      { timeout: 5000 },
    );
  });
  ...
  it("CT-AUD-04: renders UPDATE_TRANSPORT badge", async () => {
    renderAudit();
    await waitFor(
      () => expect(screen.getByText("UPDATE_TRANSPORT")).toBeInTheDocument(),
      { timeout: 5000 },
    );
  });
  ...
  it("CT-AUD-06: collapses row on Ocultar click", async () => {
    ...
    await waitFor(() =>
      expect(screen.queryByText("Estado Anterior")).not.toBeInTheDocument(),
    );
  });
  ...
  it("CT-AUD-10: badges have border class", async () => {
    ...
    expect(screen.getAllByText("CREATE_ORDER")[0].className).toContain(
      "border",
    );
  });
