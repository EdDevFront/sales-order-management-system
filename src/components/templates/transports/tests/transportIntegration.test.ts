import { saveTransportType, fetchTransportTypes } from "@/infrastructure/repositories/mockRepositories";
...
const SEED_DB = { customers: [], transportTypes: SEED_TRANSPORTS, items: [], salesOrders: [], auditLogs: [] };
...
describe("CT-INT-TRANS-01: saveTransportType creates new", () => {
  test("should add a new transport type", async () => {
    const newTransport: TransportType = { id: "trans-new", name: "Van", description: "Veículo leve para entregas urbanas" };
...
describe("CT-INT-TRANS-02: saveTransportType updates existing", () => {
  test("should update existing transport type by id", async () => {
    const updated: TransportType = { id: "trans-1", name: "Caminhão Baú", description: "Caminhão com carroceria fechada" };
...
describe("CT-INT-TRANS-04: Multiple transport types", () => {
  test("should handle multiple creates and updates", async () => {
    await saveTransportType({ id: "trans-a", name: "Type A", description: "Desc A" });
    await saveTransportType({ id: "trans-b", name: "Type B", description: "Desc B" });
    await saveTransportType({ id: "trans-1", name: "Caminhão Updated", description: "Updated" });
