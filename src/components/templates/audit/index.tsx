    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          Acompanhe operações, transições de status logísticos e alterações de
          autorização
        </p>
      </div>
      ...
      <DataTable>
        ...
        <DataTable.Body
          ...
          skeletonRows={
            <DataTable.SkeletonRows widths={AUDIT_SKELETON_WIDTHS} />
          }
        >
              ...
              <React.Fragment key={log.id}>
                <DataTable.Row>
                  ...
                  <DataTable.Cell mobileLabel="Ação">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-semibold border ${auditActionVariant(log.actionType)}`}
                    >
                      {log.actionType}
                    </span>
                  </DataTable.Cell>
                  <DataTable.Cell
                    mobileLabel="Entidade"
                    className="font-medium text-zinc-500"
                  >
                    {log.entityAffected}
                  </DataTable.Cell>
                  <DataTable.Cell
                    mobileLabel="ID Entidade"
                ...
                {isExpanded && (
                  <tr className="block md:table-row">
                    <td
                      colSpan={5}
                      className="block md:table-cell bg-zinc-50 p-4 md:p-6 dark:bg-zinc-800/20"
                    >
                      <div className="grid gap-4 md:grid-cols-2 text-xs font-mono">
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="block font-semibold uppercase tracking-wider text-zinc-400">
                              Estado Anterior
                            </span>
                            {log.previousState && (
                              <button
